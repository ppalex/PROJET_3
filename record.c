...
char* read_file(char *filename, int file) {
...
void write_file(char* content, int archive) {
...
void add_in_archive(char *filename,int file, int archive) {
	write_file(read_file(filename, file), archive);
}
	...
	while(TRUE) {
		if(file_is_modified(filename)) {
			...
			if(files_in_archive<=number) {
				...
				add_in_archive(filename, file, archive);
			}
			else {
